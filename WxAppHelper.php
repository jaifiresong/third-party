...
class WxAppHelper
{
    ...
    {
        $this->appid = 'wx24ef6cf473f84050';
        $this->secret = '9962c7b583f1ba08f01ea60d61fbc56f';
    }
    ...
    public function getAccessToken()
    {
        ...
        $token = RedisCli::select()->get($token_key);
        ...
        $rsp = FastHelper::httpGet('https://api.weixin.qq.com/cgi-bin/token?' . http_build_query($params));
        $data = json_decode($rsp, true);
        if (empty($data["access_token"])) {
            throw new \Exception('获取token失败：' . $rsp);
        }
        RedisCli::select()->set($token_key, $data['access_token'], 7000);
    ...
    public function getWxaCode($path)
    {
        ...
        $rsp = FastHelper::httpPost("https://api.weixin.qq.com/wxa/getwxacode?access_token=$token", json_encode($payload));
        if (is_string($rsp)) {
            return $rsp;
        }
    ...
    public function getWxaCodeUnlimit($scene, $page)
    {
        ...
        $token = $this->getAccessToken();
        $rsp = FastHelper::httpPost("https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=$token", json_encode($payload));
        if (is_string($rsp)) {
            return $rsp;
        }
        header('Content-type:image/jpeg');
    ...
    public function code2Session($code)
    {
        $appid = $this->appid;
        $secret = $this->secret;
        $json_data = FastHelper::httpGet("https://api.weixin.qq.com/sns/jscode2session?appid=$appid&secret=$secret&js_code=$code&grant_type=authorization_code");
        $data = json_decode($json_data, true);//openid
    }
     ...
     */
    public function mediaCheckAsync($openid, $media_url, $to_array = false)
    {
        $api = 'https://api.weixin.qq.com/wxa/media_check_async?access_token=' . $this->getAccessToken();
        $json = [
        ...
        $rsp = FastHelper::httpPost($api, json_encode($json));
        if ($to_array) {
     ...
     */
    public function msgCheck($openid, $content, $to_array = false)
    {
        $api = 'https://api.weixin.qq.com/wxa/msg_sec_check?access_token=' . $this->getAccessToken();
        $json = [
            'version' => 2,
            'openid' => $openid,
            'scene' => 2,
    ...
    public function sendSubMsg($touser, $template_id, $msg, $page = 'pages/index/index')
    {
        ...
        $rsp = FastHelper::httpPost('https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=' . $this->getAccessToken(), json_encode($data));
        return $rsp;
    }
     ...
     */
    public function urlscheme($query = '', $env = 'release', $to_array = false)
    {
        $api = 'https://api.weixin.qq.com/wxa/generatescheme?access_token=' . $this->getAccessToken();
        $json = [
            'jump_wxa' => [
